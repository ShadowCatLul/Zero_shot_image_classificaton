...
class OpenCLIP(BaseModel):
    def __init__(self, classnames, model_name="ViT-B-32", pretrained="laion2b_s34b_b79k", device="cuda" if torch.cuda.is_available() else "cpu"):
        self.device = device
        self.model, _, self.preprocess = open_clip.create_model_and_transforms(model_name, pretrained=pretrained)
        self.tokenizer = open_clip.get_tokenizer(model_name)
        self.model = self.model.to(self.device).eval()
        self.classnames = classnames
        self.prompts = [f"a photo of a {name.lower()}" for name in self.classnames]
        with torch.no_grad():
